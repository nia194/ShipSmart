...
└── types/                  Shared with @shipsmart/shared via workspace
```
...
(Frontend deps live in the workspace; nothing to install inside `apps/web`.)

### Configure

```bash
cp .env.example .env.local  # if .env.example exists; otherwise edit .env.local directly
```
...
```bash
pnpm nx build web        # production build → apps/web/dist
pnpm nx test web         # vitest run
pnpm --filter @shipsmart/web typecheck
pnpm --filter @shipsmart/web lint
```
...
- `packages/shared/` for types reused across the workspace
...
- **Echo / placeholder advisor responses**: the Python service has no
  ...
  + the matching `LLM_PROVIDER_*` flag in `apps/api-python/.env`.

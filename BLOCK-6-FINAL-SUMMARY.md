...
### 5. Use in the MCP client

1. Restart the MCP client (it reads `.mcp.json` at startup)
...
After:  MCP Server → ToolRegistry → ShippingProvider
        ^
        the MCP client, Spring Boot, etc.
```
...
**Access from:**
- the MCP client: via `.mcp.json` (automatic discovery)
...
**Access from:**
- the MCP client: via Render URL in `.mcp.json`
...
- [ ] Verify the MCP client discovers tools
- [ ] Test tool execution from the MCP client
...
4. Test the MCP client integration
...
An MCP (Model Context Protocol) server that exposes ShipSmart's tool layer to the MCP client, FastAPI, Spring Boot, and other clients via HTTP.

**"Why does it matter?"**

Previously, tools were locked inside FastAPI. Now:
- the MCP client can discover and use tools directly
...
- [x] the MCP client integration configured
...
2. **Tools not discoverable:** Check .mcp.json syntax and restart the MCP client

...
An **MCP (Model Context Protocol) server** that exposes ShipSmart's tool layer to the MCP client, FastAPI, Spring Boot, and other MCP clients.
...
✅ **MCP Client Integration** → Updated `.mcp.json` to include tools server  
...
## How the MCP Client Discovers Tools

1. **The MCP client reads `.mcp.json`** at startup
   ```json
   {
     "mcpServers": {
       "shipsmart-tools": {
...
2. **The MCP client calls `/tools/list`**
   ```bash
...
4. **The MCP client indexes tools** for discovery & autocomplete

5. **When tool is used**, the MCP client calls `/tools/call`

**Result:** Tools are discoverable and callable in the MCP client. ✅
...
- [x] the MCP client discovers tools via .mcp.json
...
- [ ] the MCP client discovers tools on Render
...
3. Verify the MCP client discovers tools
...
- the MCP client to discover and call tools
...
```
MCP client ─→ MCP Server ─→ ToolRegistry ─→ ShippingProvider
...
| **MCP Client Integration** | ✅ Ready | Updated .mcp.json |

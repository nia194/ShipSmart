...
| `api-python`   | Web Service   | Starter | `/health`            | Active    |
...
2. Ensure `uv.lock` is committed:
   ```bash
   cd apps/api-python
   # uv.lock should already be tracked
   git status uv.lock
   ```
...
See `docs/production-env-reference.md` for the complete list.
...
- `OPENAI_API_KEY` for api-python (optional — only if enabling real AI)

### Step 4: Deploy and verify

Follow the deployment order in `docs/deployment-launch-plan.md`:
1. api-java first -> verify health check
2. api-python second -> verify health check
3. web last -> verify with smoke tests

Run the smoke tests in `docs/launch-smoke-tests.md`.
...
Java and Python APIs do NOT call each other. The frontend is the sole integration point.
...
- Add an uptime monitor (e.g., UptimeRobot) pinging health endpoints
...
3. Update `VITE_JAVA_API_BASE_URL` / `VITE_PYTHON_API_BASE_URL` if APIs get custom domains
4. Rebuild the web static site after changing any `VITE_` env var
